...
parser.add_argument("-o", "--output", default="", type=str, help="output computed fft to a .csv file")
args = parser.parse_args()

fname = args.fname
#TODO: range_res shouldn't be hard coded 
range_res = 0.39
...
if args.output:
    res_str = ""
    outname_str = args.output + ".csv"

    with open(outname_str, "w") as f:
        for i in range(len(fft_result)):
            res_str = res_str + str(fft_result[i]) + ","
